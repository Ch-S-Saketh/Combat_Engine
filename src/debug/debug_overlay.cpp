...
void debug_overlay(SimulationState &sim, bool isPaused, bool isRecording, bool isPlaying){
    ...
    if (sim.num_entities > 0) DrawText(TextFormat("P1 Health: %d",sim.entities[0].health),20,50,20,RED);
    if (sim.num_entities > 1) DrawText(TextFormat("P2 Health: %d",sim.entities[1].health),20,80,20,BLUE);

    DrawText(TextFormat("Hash: %08X", hash_state(sim)), 20, 110, 20, YELLOW);

    if (isPaused) DrawText("SIM PAUSED (Press TAB to resume, F to step)", 20, 140, 20, PURPLE);
